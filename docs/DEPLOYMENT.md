...
GitHub Pages cannot apply repository-defined response headers. Configure them at a proxy/CDN if stronger production headers are required.
...
Cloudflare Pages can apply a project-specific `_headers` file, but this repository does not currently ship one. The exact regional Events API, FormLoader, cached form and form-asset origins are environment-specific. Add and test response headers/CSP for the concrete environment rather than introducing a broad policy that silently breaks registration.
