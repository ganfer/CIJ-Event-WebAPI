...
| API wrapper | Browser | Initializes and calls the Microsoft SDK using the deployed browser configuration |
...
2. `api-wrapper.js` initializes the vendored Microsoft SDK.
...
2. The browser passes the query value to the SDK, which encodes it into the request path.
3. The browser requests event details.
4. Rich text is parsed with an element/attribute allowlist.
5. HTTP(S) event and speaker images may be loaded when supplied by the API.
6. Optional sessions and speakers load without blocking the core page.

### Registration

1. The event response contains a Microsoft form placeholder and loader script.
2. The app parses the embed and clones its top-level non-script elements.
3. External and inline scripts in that embed are recreated in the page.
4. FormLoader fetches and submits the form directly to Microsoft.

This sample-compatible behavior is intentionally retained after a previous stricter reconstruction caused a functional regression. It means event/form embed content is a trusted administrative boundary and remains a documented security risk.
...
None. There is no Worker runtime, Pages Function, KV, D1, R2, Cache API, Wrangler configuration or binding.

### Portable business/presentation logic

Rendering, localization and filtering are plain JavaScript. The Node-only outbound URL policy is isolated in `scripts/lib/http.mjs`. Static hosting can move unchanged to Azure Static Web Apps, object storage/CDN, Nginx, Apache or another static platform.
