...
sequenceDiagram
    ...
    E-->>B: Event + Microsoft form embed metadata
...
The app reads event metadata, a detail model, sessions and speakers. It does not implement its own registration endpoint. For compatibility with the Microsoft sample, it copies the returned form placeholder and external loader script into the page; Microsoft's loader then fetches and submits the form. Cancellation, re-registration, session selection and attendee portal functions are not implemented by repository code.
...
| `public/js/api-wrapper.js` | Sample-compatible Events API access |
| `public/js/event-grid.js` | List rendering, search and sort |
| `public/js/event-grid-translations.js` | Compatibility integration for localized event cards |
| `public/js/event-details.js` | Detail, rich text, sessions, speakers and Microsoft form embed |
| `public/js/event-details-translations.js` | Compatibility integration for localized event details |
| `public/js/event-details-runtime-fix.js` | Keeps event/form rendering independent from optional session/speaker calls |
...
- Form does not load: use the current Microsoft embed code and verify the FormLoader and form endpoints in browser network tools.
