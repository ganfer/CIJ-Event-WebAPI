...
Confirm `public/js/config.js` exists locally and contains all required values. Run `npm run check` for syntax/reference errors. Use the Customer Insights Events API base supplied by Microsoft.
...
Check that `id` is present and is the event's readable ID. The retained sample-compatible wrapper can map several API/network failures to `null`, so inspect both the visible API error and browser network status before concluding that an event does not exist.

## Registration form absent or does not load

- Confirm the event has a published form.
- Regenerate the embed with current Customer Insights endpoints.
- Verify the returned holder has `data-form-id`, `data-form-api-url`, `data-cached-form-url` and `data-readable-event-id`.
- Verify the returned FormLoader script loads successfully.
- Allow the final site domain for external form hosting.

The browser currently preserves Microsoft's returned embed for compatibility. Do not replace it with a stricter reconstruction without testing the real regional embed end to end.
...
## Long web-application token fails after a code change

The Customer Insights token is opaque. Do not trim, truncate or impose an arbitrary 200-character limit. Run the runtime regression suite and confirm the exact token reaches the Microsoft SDK unchanged without printing it.
